...
class LineDetector():
		...
		if self.below_thershold:
			...
			if self.raw_value < self.lo_thresh:
				...
				self.lo_thresh = self.lo_avg + \
							self.average_margin * (self.thershold - self.lo_avg)
			...
			if self.raw_value < self.hi_thresh:
				...
				self.hi_thresh = self.hi_avg - \
							self.average_margin * (self.hi_avg - self.thershold)
	...
	def line_detected(self):
		...
		self.below_thershold = self.raw_value < self.thershold

		if line_low:
			return self.below_thershold
		else:
			return not self.below_thershold

	def _calculate_normalised(self):

		norm = (self.raw_value - self.lo_avg)/(self.hi_avg - self.lo_avg)*100
		norm = norm if norm < 100 else 100
		norm = nrom if norm > 0 else 0
